                ...
                opt.street.autocomplete(serviceUrl, {
                    ...
                    formatItem: function(data, i, max, value) {
                        ...
                        for( var k=0; k < links.length; k++ ) {
                            var line  = links[k].split(",");
                            var city = line[1];
                            var zip = line[2];

                            if( locations[city] )
                                locations[city].push(zip);
                            else
                                locations[city] = [zip];
                        }
                        var cities = [];
                    ...
                    highlight: function(value, term) {
                        var text = value.find("span:eq(0)").html();
                        value.find("span:eq(0)").html( $.Autocompleter.defaults.highlight(text, term) );
                        return value;
                    },
                    changeTerm: function(term) {
                        var street_words = term.split(/\s+/);
                        if ( street_words.length > 1 && street_words[0].match(getStreetTypeRegexp()) )
                            street_words.shift();
                        return street_words.join(" ");
                    }
                });
...
$.Autocompleter = function(input, options) {
	...
	function onChange(crap, skipPrevCheck) {
		...
		var currentValue = options.changeTerm( $input.val() );
...
$.Autocompleter.defaults = {
	...
	extraParams: {},
	selectFirst: true,
    changeTerm: function(term) { return term; },
